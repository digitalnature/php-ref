    ...
    class ref
    {
        ...
        public static function parseComment($comment, $key = null)
        {
            ...
            foreach ($comment as $line) {
                ...
                if (!in_array($tag, ['global', 'param', 'return', 'var'])) {
                    $tags[$tag][] = $line;
                    end($tags[$tag]);
        ...
        public static function splitRegex($pattern)
        {
            ...
            preg_match_all('/\[\^?]?(?:[^\\\\\]]+|\\\\[\S\s]?)*]?|\\\\(?:0(?:[0-3][0-7]{0,2}|[4-7][0-7]?)?|[1-9][0-9]*|x[0-9A-Fa-f]{2}|u[0-9A-Fa-f]{4}|c[A-Za-z]|[\S\s]?)|\((?:\?[:=!]?)?|(?:[?*+]|\{[0-9]+(?:,[0-9]*)?\})\??|[^.?*+^${[()|\\\\]+|./',
                    $pattern, $matches);
            ...
            foreach ($matches as $m) {
                ...
                if ($m[0] === '[') {
                        ...
                        if ($cm[0] === '\\') {
                            ...
                            if ($cLastRangeable) {
                                ...
                                if ($nextToken) {
                                    ...
                                    if ((!is_null($nextTokenCharCode) && $lastCC > $nextTokenCharCode) || $cLastType === 2 ||
                                            preg_match('/^\\\\[dsw]$/i', $nextToken[0])
                                    ) {
                    ...
                    if (isset($m[1]) && preg_match('/^[1-9]/', $m[1])) {
                        ...
                        if ($num > 0) {
                            $output[] = ['meta' => "\\{$num}", 'text' => $nonBackrefDigits];

                        } else {
                            preg_match('/^\\\\([0-3][0-7]{0,2}|[4-7][0-7]?|[89])([0-9]*)/', $m, $pts);
                            $output[] = ['meta' => '\\'.$pts[1], 'text' => $pts[2]];
                        }
        ...
        public static function getInputExpressions(array &$options = null)
        {
            ...
            foreach ($tokens as $i => $token) {
                ...
                if (isset($tokens[$i - 1]) && is_array($tokens[$i - 1]) && in_array($tokens[$i - 1][0], [T_DOUBLE_COLON, T_OBJECT_OPERATOR], true)) {
                    continue;
                }
                ...
                if (isset($tokens[$i + 1]) && ($tokens[$i + 1][0] === '(')) {
                    ...
                    if ($options !== null) {
                        $j = $i - 1;
                        while (isset($tokens[$j]) && is_string($tokens[$j]) && in_array($tokens[$j], ['@', '+', '-', '!', '~'])) {
                            $options[] = $tokens[$j--];
                        }
                    }
        ...
        protected function fromReflector(\Reflector $reflector, $single = '', \Reflector $context = null)
        {
            ...
            foreach ($items as $item) {
                ...
                $comments = $item->isInternal() ? [] : static::parseComment($item->getDocComment());
                $meta     = ['sub' => []];
                $bubbles  = [];

                if ($item->isInternal()) {
                    $extension     = $item->getExtension();
                    $meta['title'] = ($extension instanceof \ReflectionExtension) ? sprintf('Internal - part of %s (%s)', $extension->getName(),
                            $extension->getVersion()) : 'Internal';
                    ...
                    $meta['sub'][] = ['Defined in', basename($item->getFileName()).':'.$item->getStartLine()];
                }
                ...
                if (($item instanceof \ReflectionFunction) || ($item instanceof \ReflectionMethod)) {
                    if (($context !== null) && ($context->getShortName() !== $item->getDeclaringClass()->getShortName())) {
                        $meta['sub'][] = ['Inherited from', $item->getDeclaringClass()->getShortName()];
                    }

                    if ($item instanceof \ReflectionMethod) {
                        try {
                            $proto         = $item->getPrototype();
                            $meta['sub'][] = ['Prototype defined by', $proto->class];
                        } catch (\Exception $e) {
                        }
                    }
        ...
        protected function linkify(\Reflector $reflector, $constant = null)
        {
            ...
            if ($parent->isInternal()) {
                ...
                if (in_array($type, ['method', 'property'], true)) {
        ...
        protected function evaluate(&$subject, $specialStr = false)
        {
            ...
            switch ($type = gettype($subject)) {
                ...
                case 'string':
                    ...
                    if ($specialStr) {
                        $this->fmt->sep('"');
                        $this->fmt->text(['string', 'special'], $subject, "string({$info})");
                        $this->fmt->sep('"');
                        return;
                    }
                    ...
                    if (static::$config['showStringMatches'] && ($length > 2) && (trim($subject) !== '')) {
                        ...
                        //$isFile = ($length < 2048) && (max(array_map('strlen', explode('/', str_replace('\\', '/', $subject)))) < 128) &&
                        //        !preg_match('/[^\w\.\-\/\\\\:]|\..*\.|\.$|:(?!(?<=^[a-zA-Z]:)[\/\\\\])/', $subject);

                        //if ($isFile) {
                        if (is_file($subject) || is_dir($subject)) {
                            try {
                        ...
                        if (!$isNumeric && ($length > 4)) {
                            ...
                            if (static::$config['showUrls'] && static::$env['curlActive'] && filter_var($subject, FILTER_VALIDATE_URL)) {
                                ...
                                if ($nfo['http_code']) {
                                    ...
                                    $this->fmt->text('url', sprintf('%s:%d %s %.2fms (%d)', !empty($nfo['primary_ip']) ? $nfo['primary_ip'] : null,
                                            !empty($nfo['primary_port']) ? $nfo['primary_port'] : null, $contentType[0], $nfo['total_time'], $nfo['http_code']));
                                    $this->fmt->endContain();
                                }
                            ...
                            if ($length < 128 && (bool)$timestamp && checkdate(date('m', $timestamp), date('d', $timestamp), date('Y', $timestamp)) &&
                                    static::$env['supportsDate'] && !preg_match('/[^A-Za-z0-9.:+\s\-\/]/', $subject)
                            ) {
                                try {
                            ...
                            $isSerialized = ($unserializing < 3) && (($subject[$length - 1] === ';') || ($subject[$length - 1] === '}')) &&
                                    in_array($subject[0], ['s', 'a', 'O'], true) &&
                            ...
                            $isJson = !$isSerialized && ($decodingJson < 3) && in_array($subject[0], ['{', '['], true);
            ...
            $traits     = static::$env['is54'] ? $reflector->getTraits() : [];
            ...
            if (static::$config['showIteratorContents'] && $reflector->isIterateable()) {
                ...
                foreach ($itContents as $key => $value) {
                    ...
                    $this->fmt->startRow();
                    $this->fmt->text(['key', 'iterator'], $key, sprintf('Iterator key: %s', $keyInfo));
                    $this->fmt->colDiv();
                    $this->fmt->sep('=>');
                    $this->fmt->colDiv();
                    $this->evaluate($value);
                ...
                foreach ($constants as $name => $value) {
                    ...
                    foreach ($parents as $parent) {
                        if ($parent->hasConstant($name)) {
                            if ($parent !== $reflector) {
                                $type[] = 'inherited';
                                $meta   = ['sub' => [['Prototype defined by', $parent->name]]];
                            }
                            break;
                        }
                    }
            ...
            if ($props) {
                ...
                foreach ($props as $idx => $prop) {
                    ...
                    if ($meta) {
                        if ($inherited) {
                            $meta['sub'] = [['Declared in', $sourceClass->getShortName()]];
                        }
            ...
            if ($methods && !$this->hasInstanceTimedOut()) {
                ...
                foreach ($methods as $idx => $method) {
                    ...
                    $paramCom   = $method->isInternal() ? [] : static::parseComment($method->getDocComment(), 'tags');
                    $paramCom   = empty($paramCom['param']) ? [] : $paramCom['param'];
    ...
    class RTextFormatter extends RFormatter
    {
        ...
        protected
             ...
             */
                ...
                $lastIdx = 0, $lastLineSt = 0, $levelPad = [0];
